...
pub fn spec_version() -> SbiSpecVersion {
    let value = opensbi_call(SBI_BASE_EID, SBI_GET_SPEC_VERSION_FID, 0, 0, 0, 0, 0, 0).unwrap();
    SbiSpecVersion {
        major: (value >> 24) & 0x7f,
...
pub fn impl_id() -> SbiImplId {
    let value = opensbi_call(SBI_BASE_EID, SBI_GET_IMPL_ID_FID, 0, 0, 0, 0, 0, 0).unwrap();
    value.into()
}

#[allow(unused)]
pub fn impl_version() -> usize {
    opensbi_call(SBI_BASE_EID, SBI_GET_IMPL_VERSION_FID, 0, 0, 0, 0, 0, 0).unwrap()
}
