...
use crate::fs::{open, File, OpenFlags};
...
pub fn add_initproc_into_fs() {
    ...
    if let Some(inode) = open("/", "initproc", OpenFlags::RDONLY) {
        println!("Already have initproc in FS");
        inode.delete();
    }
    if let Some(inode) = open("/", "user_shell", OpenFlags::RDONLY) {
        println!("Already have init user_shell in FS");
        inode.delete();
    }

    //Write apps initproc to disk from mem
    if let Some(inode) = open("/", "initproc", OpenFlags::CREATE) {
    ...
    if let Some(inode) = open("/", "user_shell", OpenFlags::CREATE) {
        let mut data: Vec<&'static mut [u8]> = Vec::new();
        data.push(unsafe {
